    ...
    .then((response)=>{
        ...
        if(data.data){
            ...
            for ( let i = 0 ; i< data.data.length ;i++){
                ...
                donorDataContainer.innerHTML += `
                <div class="donor-card ${item.e_ready === 1 ? "emergency-card":""}" onclick="openDonationDetailsDialogBox(${item.id})">
...
function openDonationDetailsDialogBox(targetId){
    ...
    if (selectedRequestTab.innerHTML.split(" ")[0]==="Signup"){
        ...
        <div class="section-1">
          ...
          <div>
            ...
            <div style="display: flex; justify-content: space-between; width: 100%; align-items: center;">
              ...
              ${age}, ${clickedDonor.gender === 'M' ? '<i class="fas fa-mars" style="color: darkblue; scale: 1.1; text-shadow: 1px 2px 2px rgba(255,255,255,0.7);"></i>' : '<i class="fas fa-venus" style="color:palevioletred; scale: 1.1; text-shadow: 1px 2px 2px rgba(0,0,0,0.7);"></i>'} 
              </div>
            </div>
          </div>
        </div>
        <div class="section-2">
          ...
          <div style="padding: 5px;">
            ...
            <div style="display: flex; justify-content: space-between; gap: 20px;">
              ...
              <div id="donor_last_donated" ${!clickedDonor.last_donation ? 'style="width:53%; text-align:center;"' : ''} >${clickedDonor.last_donation ? formatDate(clickedDonor.last_donation) : '---'}</div>
            </div>
          </div>
        </div>
        <div class="section-3">
        ...
        ${isDonorAvailable ? (clickedDonor.e_ready ? '<div id="donor_emergency_label" >Emergency Available</div>' : '') : '<div id="not_available_label" >Not Available</div>'}
        ...
        <div class="request_dialog_button" style="display: flex; justify-content: center; margin-top: 10px; gap:20px">
              ${isRequestMailAllowed ? `<div class="request_blood_btn" onclick="confirmSendEmail(${clickedDonor.id})">Request Blood <i class="fas fa-paper-plane"></i></div>` : ''}
